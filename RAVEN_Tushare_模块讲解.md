...
| 股票池 | HS300 |
| 数据频率 | 日频 |
| 数据区间 | 2018-01-01 至 2024-12-31 |
| 训练集 | 2018-01-01 至 2021-12-31 |
| 验证集 | 2022-01-01 至 2022-12-31 |
| 测试集 | 2023-01-01 至 2024-12-31 |
| 最大历史窗口 | 120 个交易日 |
| Patch 长度 | 16 个交易日 |
| 专家数 | 3 |
| CIT 阈值 | 0.3、0.6、0.9 |
...
~~~text
训练：2018-01-01 至 2021-12-31
验证：2022-01-01 至 2022-12-31
测试：2023-01-01 至 2024-12-31
~~~

该默认切分使用当前已验证可获取的历史数据；若 Tushare 覆盖到更晚日期，可再通过命令行参数扩展样本外区间。模型根据验证集损失保存最佳状态，测试集只用于最终评估，不能用于调参。
